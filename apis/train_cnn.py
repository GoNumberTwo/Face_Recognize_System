...
class Dataset:
    def __init__(self):
        ...
        #数据集加载路径
        self.user_path = './user_faces'
        self.others_path = './others_faces'
...
class Model:
    ...
    def build_model(self, dataset, nb_classes = 2):
        ...
        self.model.add(Conv2D(32, (3, 3), padding='same', input_shape = dataset.input_shape))#2维卷积层
        self.model.add(Activation('relu'))                                                            #激活函数

        self.model.add(Conv2D(32, 3, 3))                                                        #卷积层
        self.model.add(Activation('relu'))                                                             #激活函数

        self.model.add(MaxPooling2D(pool_size= (2, 2)))#池化层
        self.model.add(Dropout(0.25))#dropout层

        self.model.add(Conv2D(64,(3,3), padding = 'same'))#卷积层
        self.model.add(Activation('relu'))#激活函数

        self.model.add(Conv2D(64,(3,3)))#卷积层
        self.model.add(Activation('relu'))#激活函数
    ...
    def Train(self, dataset, batch_size = 20, nb_epoch = 30, data_augmentation = True):
        ...
        self.model.fit(
            datagen.flow(dataset.train_imgs, dataset.train_labels, batch_size=batch_size),
            steps_per_epoch=dataset.train_imgs.shape[0] // batch_size,
            epochs=nb_epoch,
            validation_data=(dataset.val_imgs, dataset.val_labels)
        )
    ...
    def face_predict(self,img):
            if K.image_data_format() == 'channels_first' and img.shape !=(1,3, size, size):
                img = img.reshape((1,3,size,size))
            elif K.image_data_format() != 'channels_first' and img.shape != (1, size, size, 3):
